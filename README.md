...
# 😉 FreeReels API — by CUTAD

**Short movie gratis dengan subtitle**

[![Provider](https://img.shields.io/badge/provider-freereels-22c55e?style=for-the-badge)](https://www.cutad.web.id/category/freereels)
...
Unlock akses ke **FreeReels** dan 9 provider streaming lainnya dengan **satu API key**.
...
- 🎯 **Satu endpoint, banyak sumber** — `FreeReels` + 9 provider lain tanpa ubah kode.
...
**cURL:**
```bash
curl "https://www.cutad.web.id/api/public/freereels?action=rank" \
...
const res = await fetch(
  "https://www.cutad.web.id/api/public/freereels?action=rank",
...
import os, requests
r = requests.get(
    "https://www.cutad.web.id/api/public/freereels",
    params={"action": "rank"},
...
curl -o cutad-client.mjs https://raw.githubusercontent.com/rudiansyah1998/freereels-api/main/src-js/client.mjs
```
```js
import { CutadClient } from "./cutad-client.mjs";
const client = new CutadClient(process.env.CUTAD_KEY, "freereels");
const items = await client.rank();
```

### Python
```bash
curl -o cutad_client.py https://raw.githubusercontent.com/rudiansyah1998/freereels-api/main/src-py/cutad_client.py
```
```python
from cutad_client import CutadClient
client = CutadClient(api_key=os.environ["CUTAD_KEY"], provider="freereels")
items = client.rank()
```
...
Base URL: `https://www.cutad.web.id/api/public/freereels`
...
|-----------|--------|---------------------|---------------------------------------------------|
| `rank`    | GET    | —                   | Konten populer / trending dari FreeReels           |
...
- **Aggregator / discovery** — gabungkan FreeReels dengan 9 provider lain dalam 1 dashboard.
...
<p>Ya. Response <code>action=stream</code> mengandung field <code>subtitles[]</code> dengan URL VTT (auto-converted dari SRT upstream). Bahasa tersedia tergantung provider — FreeReels biasanya punya sub Indonesia, Inggris, dan bahasa lainnya.</p>
</details>
